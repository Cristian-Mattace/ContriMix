"""Inference for Intrabatch permutation DRIT."""
import logging
...
def main() -> None:
    ...
    pretrain_model_path = checkpoint_folder / (
        ...
        + "consistency_weight_10.0_attr_consistency_weight_1.0_mode_seeking_loss_weight_5.0_content_channel_covariance_"
        + "loss_weight_0.2_MIDOG_v1.3/every_n/"
        + "periodic_epoch=39_val_encoders_generators_total_loss=2.758_train_encoders_generators_total_loss=2.569_train_"
        + "disc_total_loss=0.636.ckpt"
    )
    ...
    enc_a = load_trained_model_from_checkpoint(
        pretrain_model_path,
        network=AttributeEncoder(in_channels=3, out_channels=8, num_stain_vectors=NUM_STAIN_VECTORS),
        starts_str="_enc_a.",
    ).eval()
    gen = load_trained_model_from_checkpoint(
        pretrain_model_path, network=AbsorbanceImGeneratorWithConvTranspose(in_channels=8), starts_str="_gen."
    ).eval()

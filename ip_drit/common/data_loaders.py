...
import numpy as np
import torch
from ip_drit.datasets import AbstractPublicDataset, SubsetPublicDataset
...
def split_into_groups(g: torch.Tensor) -> Tuple[torch.Tensor, List[torch.Tensor], torch.Tensor]:
    ...
    Args:
        g: Vector of groups
    Returns:
        groups: Unique groups present in g
        group_indices: A list of Tensors, where the i-th tensor is the indices of the elements of g that equal groups[i].
            Has the same length as len(groups).
        unique_counts: Counts of each element in groups. Has the same length as len(groups).
    """

...
def _construct_colormap_list(num_colors: int, colormap, device: str) -> torch.Tensor:
    return torch.tensor([[x for x in colormap(idx)[:3]] for idx in range(num_colors)], device=device, dtype=torch.float32).T

def _convert_content_tensor_to_8_bit(zc: torch.Tensor) -> torch.Tensor:
    zc = zc.unsqueeze(1)  # [C, 1, H, W]
    min_val = zc.view(*zc.shape[:2], -1).min(dim=-1, keepdim=True)[0].unsqueeze(-1)
    max_val = zc.view(*zc.shape[:2], -1).max(dim=-1, keepdim=True)[0].unsqueeze(-1)    

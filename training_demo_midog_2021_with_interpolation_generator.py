...
The image generator uses the interpolation-based generator named AbsorbanceImGeneratorWithInterpolation.
This generator is expected to reduce the gridding artifact.
"""
import logging
...
def _train_model(sample_lists_by_domain_idx: Dict[int, List[Sample]]) -> None:
    ...
    BATCH_SIZE_PER_GPU = nearest_even_minibatch_size(
        org_minibatch_size=8
    )  # 16 is the largest number that does not cause OOM but 14 seems to be much more stable.
    ...
    experiment_name = (
        ...
        + f"bs_{BATCH_SIZE_PER_GPU}_samples_w_{loss_string_from_weight_dict(loss_weights_by_name)}_MIDOG_v1.2"
    )
    ...
    training_module = MutliClassTrainingModule(
        ...
        generator_type=GeneratorType.WithInterpolation,
